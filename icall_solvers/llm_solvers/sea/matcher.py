...
class SeaMatcher(BaseLLMSolver):
    ...
    def process_callsite_target(self, parent_func_name: str, callsite_text: str, func_pointer_summary: str,
        ...
        # 没有消融callee local context
        if self.ablation_type != 3:
        ...
        # 合成caller local summary
        if icall_summary != "":
            icall_summary_ = User_ICall_local.format(parent_func_name=parent_func_name,
                                                     icall_summary=icall_summary)
        else:
            icall_summary_ = ""

        # 合成caller global summary
        if func_pointer_summary != "":
            func_pointer_summary_ = User_Func_Pointer.format(func_pointer_summary)
        else:
            func_pointer_summary_ = ""

        # 合成callee local summary
        if func_summary != "":
            func_summary_ = User_Func_Local.format(func_summary=func_summary)
        else:
            func_summary_ = ""

        if icall_summary_ != "" or func_pointer_summary_ != "":
            icall_title = "## 1.summary of indirect-call:"
        else:
            icall_title = ""

        if func_summary_ != "" or target_addr_summary_ != "":
            func_title= "## 2.summary of target function {func_name}:".format(func_name=func_name)
        else:
            func_title = ""
        ...
        user_prompt_match: str = User_Match.format(icall_expr=callsite_text,
                                                   ...
                                                   icall_summary=icall_summary_,
                                                   func_title=func_title,
                                                   func_summary=func_summary_,
                                                   func_name=func_name,
